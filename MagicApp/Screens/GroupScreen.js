...
export default class GroupScreen extends React.Component {
              ...
              if (snapshot.val() == null) {
                ...
                root
                  ...
                  .catch(function(error) {
                    alert(
                      "Data could not be loaded. Try reopening app." + error
                    );
                  });
              } else {
                var updates = {};
    ...
    root.ref("groups").on("value", function(snapshot) {
      ...
      for (var i = 0; i < snapshot.val().length; i++) {
        ...
        var check_date = new Date(
          parseInt(full_date.substring(0, 4)),
          parseInt(full_date.substring(8, 10)) - 1,
          parseInt(full_date.substring(5, 7))
        );
  ...
  loadFriends(my, key, numPeople, people, joined) {
    if (
      !my.state.friendData ||
      my.state.friendData.length != numPeople ||
      key != my.state.key
    ) {
      my.setState({
        friendData: [{ key: "0", name: "Loading Patel", prompt: "Loading" }]
      });
      var counter = 1;
      ...
      for (var i = 0; i < numPeople; i++) {
        firebase
          .database()
          .ref("users/" + Object.keys(people)[i])
          .on(
            "value",
            function(snapshot) {
              if (snapshot.key) {
  ...
  renderGroupFlatListCell = ({ item }) => {
    return (
      <Group
        item={item}
        nav={this.props.nav}
        updateModalDet={this.updateDetailModal}
    ...
    Linking.canOpenURL("https://wbc-magic.weebly.com/report").then(
      supported => {
        if (supported) {
          ...
          console.log(
            "Don't know how to open URI: https://wbc-magic.weebly.com/report"
          );
        }
      }
    );
    ...
    return (
      <View style={styles.groupsView}>
        ...
        <Modal
          ...
          onRequestClose={() => {
            this.setDetailModalVisible(!this.state.modalDetailVisible);
        ...
        >
          <View style={styles.menuView}>
            ...
            <View style={styles.restHeaderView}>
              <View style={{ height: hp("7%") }}>
                <ScrollView horizontal={true}>
                  <Text style={styles.theTandoorText}>
                    {this.state.item.location_name +
                      " | " +
                      this.state.item.group_name}
                  </Text>
          ...
          >
            <View style={styles.artboard2View}>
              <View
                style={{
                  flexDirection: "row"
                }}
              ...
              <View style={styles.contentView}>
                <View style={styles.formView}>
                  ...
                  <View style={styles.edittextTextonlyPlaceholderTwoView}>
                    <Text style={styles.paymentTwoText}>Intro</Text>
                    <View
                      style={{
                        flex: 1,
                        justifyContent: "flex-end"
        ...
        >
          ...
          >
            ...
            >
              <Text style={styles.miscBigButtonButtonText}>Create</Text>
            </TouchableOpacity>
          </View>
        </View>

        <View style={styles.groupFlatListViewWrapper}>
        ...
        >
          <Text style={styles.miscReportButtonButtonText}>Report</Text>
        </TouchableOpacity>
      </View>
    );
  }
